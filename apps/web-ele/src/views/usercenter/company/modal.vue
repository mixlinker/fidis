...
import { addCompanyApi, editCompanyApi, getCompanyListApi } from '#/api';
  ...
  onConfirm: async () => {
    if (FormRef.value) {
      ...
      await (modalType.value === 'add'
        ? addCompanyApi(Form.value)
        : editCompanyApi(Form.value));
...
<template>
  ...
  >
    ...
    >
      ...
      <el-form-item :label="$t('page.company.parent-company')" prop="parent_id">
